...
static void bitmap_set(uint64_t frame)   { bitmap[frame / 8] |=  (1 << (frame % 8)); }
static void bitmap_clear(uint64_t frame) { bitmap[frame / 8] &= ~(1 << (frame % 8)); }
static int  bitmap_test(uint64_t frame)  { return bitmap[frame / 8] &  (1 << (frame % 8)); }
...
static uint64_t find_bitmap_region(struct MB2_TAG_MMAP *mmap, uint8_t *end,
    ...
    for (struct MB2_MMAP_ENTRY *e = mmap->entries;
         (uint8_t*)e < end; e = (struct MB2_MMAP_ENTRY*)((uint8_t*)e + mmap->entry_size)) {
...
void pmm_init(uint64_t mb2_addr) {
    struct MB2_TAG_MMAP *mmap = (struct MB2_TAG_MMAP*)mb2_find_tag(mb2_addr, MULTIBOOT_TAG_MMAP);
    if (!mmap) {
        log_write("PMM: no mmap tag", KERNEL, LOG_ERROR);
        for (;;) __asm__ volatile ("cli; hlt");
    }
    // find highest USABLE address (skip high reserved MMIO holes)
    ...
    for (struct MB2_MMAP_ENTRY *e = mmap->entries;
        ...
        if (top > highest_usable_addr) highest_usable_addr = top;
    }
    ...
    // Keep the bitmap away from the kernel and from GRUB's Multiboot info block.
    uint64_t mb2_size = *(uint32_t*)mb2_addr;
    uint64_t min_bitmap_addr = max_u64((uint64_t)_kernel_end, mb2_addr + mb2_size);
    ...
    // exclude every multiboot module range from bitmap placement
    {
        ...
        while (1) {
            ...
            if (t->type == MULTIBOOT_TAG_END) break;
    ...
    uint64_t bitmap_phys = find_bitmap_region(mmap, end, bitmap_bytes, min_bitmap_addr);
    if (bitmap_phys == 0) {
    ...
    for (struct MB2_MMAP_ENTRY *e = mmap->entries;
         (uint8_t*)e < end; e = (struct MB2_MMAP_ENTRY*)((uint8_t*)e + mmap->entry_size)) {
        if (e->type == 1) mark_region_free(e->base, e->len);
    }

    // re-mark unusable regions
    mark_region_used(0, 0x100000);                              // first 1MiB (BIOS)
    ...
    // mark every multiboot module so its memory is preserved
    {
        uint8_t *p = (uint8_t*)mb2_addr + 8;
        while (1) {
            struct MB2_TAG *t = (struct MB2_TAG*)p;
