...
static inline uint16_t to_be16(uint16_t value) { return bswap16(value); }
static inline uint16_t from_be16(uint16_t value) { return bswap16(value); }
static inline uint32_t to_be32(uint32_t value) { return bswap32(value); }
static inline uint32_t from_be32(uint32_t value) { return bswap32(value); }
...
static inline int ipv4_addr_equal(const uint8_t *a, const uint8_t *b) {
  return a[0] == b[0] && a[1] == b[1] && a[2] == b[2] && a[3] == b[3];
}

static inline int ipv4_addr_is_zero(const uint8_t *a) {
  return (a[0] | a[1] | a[2] | a[3]) == 0;
}

/* Broadcast for `ip` under `mask`: every host bit set. */
static inline int ipv4_is_broadcast(const uint8_t *addr, const uint8_t *ip,
                                    const uint8_t *mask) {
...
static inline int ipv4_same_subnet(const uint8_t *a, const uint8_t *b,
                                   const uint8_t *mask) {
  for (int i = 0; i < IPV4_ALEN; i++) {
