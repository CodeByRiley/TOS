...
usize vfs_write(struct vfs_file *file, const void *buffer, usize length) {
    const struct vfs_operations *ops = file_ops(file);
    return ops && ops->write
               ? ops->write(file->mount->fs, file, buffer, length)
               : 0;
}
...
int vfs_stat(const char *path, struct vfs_stat *out) {
    ...
    const struct vfs_operations *ops =
        mount ? mount->filesystem->operations : 0;
...
long vfs_read_dir_one(const char *path, u32 *index,
    ...
    const struct vfs_operations *ops =
        mount ? mount->filesystem->operations : 0;
    return ops && ops->read_dir
               ? ops->read_dir(mount->fs, relative, index, out)
               : -1;
}
...
long vfs_read_dir(const char *path, u32 *index, char *buffer,
    ...
    for (;;) {
        ...
        usize required = name_length + 1 +
                          (entry.type == VFS_NODE_DIRECTORY ? 1 : 0);
