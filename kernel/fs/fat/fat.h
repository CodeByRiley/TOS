/* kernel/fs/fat/fat.h , minimal FAT filesystem driver.
 *
 ...
 * disk is kernel/fs/fat/ahci/fat_ahci.c's job, which plugs into the write-through
...
struct fat_file {
  ...
  u32 pos;
  struct dir_entry
      *dir_ent; /* pointer in fs_image; NULL if read-only/unlinked */
};
...
usize fat_read(struct fat_file *f, void *buf, usize len);
usize fat_write(struct fat_file *f, const void *buf, usize len);
int fat_seek(struct fat_file *f, u32 pos);
...
long fat_read_dir(const char *path, u32 *index, char *buf, usize len);
long fat_read_root_dir(u32 *index, char *buf, usize len);
long fat_read_dir_one(const char *path, u32 *index, char *buf, usize len,
                      int *is_dir);
...
typedef void (*fat_sector_writer)(u32 lba, void *sector_data);
...
usize fat_volume_size(const void *boot_sector,
                       u32 *bytes_per_sector_out);
#endif
