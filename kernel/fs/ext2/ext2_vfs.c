...
static u8 vfs_type(const struct ext2_inode *inode) {
  return (inode->mode & EXT2_S_IFMT) == EXT2_S_IFDIR ? VFS_NODE_DIRECTORY
                                                     : VFS_NODE_FILE;
}
...
static int ext2_backend_create(void *context, const char *path,
  ...
  struct ext2_inode *inode = ext2_inode_get(fs, inode_number);
  if (!inode ||
      ext2_dir_add(fs, parent, name, inode_number, EXT2_FT_REG_FILE) != 0) {
    if (inode)
...
static void set_directory_entry(struct ext2_dir_entry *entry, u32 inode_number,
                                u16 record_length, const char *name,
                                u8 name_length) {
...
static int ext2_backend_mkdir(void *context, const char *path) {
  ...
  set_directory_entry((struct ext2_dir_entry *)block, inode_number, 12, ".", 1);
...
static long ext2_backend_read_dir(void *context, const char *path, u32 *index,
...
void ext2_vfs_register(void) {
  log_write("ext2: initialising VFS", KERNEL, LOG_INFO);
