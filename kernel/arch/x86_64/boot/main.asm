...
start:
  cli                        ; we usually get IF,DF = 0 from grub, I dont trust it
  cld                        ; so we do it ourselves
  mov esp, stack_top         ; set up stack
  mov [mb2_save], ebx        ; stash mb2 ptr (survives clear/cpuid clobbers)
  mov [magic_save], eax      ; eax dies in serial_init; the magic must not
...
.hhdm_entry_loop:
  ...
  ; edx carries the physical high dword and the NX bit together.
  ; This adc increments the physical high bits when eax wraps at 4GiB.
  ; The bitwise OR above safely reapplies the NX bit every iteration.
  add eax, 0x200000                       ; next 2 MiB physical range
  adc edx, 0                              ; increment physical high bit if eax wrapped
  add edi, 8
  dec ecx
  jnz .hhdm_entry_loop
...
.wait:
  pause                      ; prevent spinlock pipeline stall
  mov dx, 0x3FD              ; LSR
  in al, dx
...
; 64 KiB bootstrap stack for kernel_main and early initialization.
stack_bottom:
  resb 4096 * 16
stack_top:
mb2_save:
