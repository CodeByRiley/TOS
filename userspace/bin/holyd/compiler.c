...
static void compile_binary_op(Compiler *compiler, BytecodeChunk *chunk,
  ...
  case TOKEN_PLUS:
    ...
    break;
  case TOKEN_MINUS:
    emit_simple(chunk, BC_SUB);
    break;
  case TOKEN_STAR:
    emit_simple(chunk, BC_MUL);
    break;
  case TOKEN_SLASH:
    emit_simple(chunk, BC_DIV);
    break;
  case TOKEN_EQEQ:
    emit_simple(chunk, BC_EQ);
    break;
  case TOKEN_NEQ:
    emit_simple(chunk, BC_NE);
    break;
  case TOKEN_LT:
    emit_simple(chunk, BC_LT);
    break;
  case TOKEN_GT:
    emit_simple(chunk, BC_GT);
    break;
  case TOKEN_LTEQ:
    emit_simple(chunk, BC_LE);
    break;
  case TOKEN_GTEQ:
    emit_simple(chunk, BC_GE);
    break;
  case TOKEN_TILDE:
    emit_simple(chunk, BC_CONCAT);
    break;
  default:
...
static void compile_statement(Compiler *compiler, BytecodeChunk *chunk,
  ...
  case AST_VAR_REF:
    if (program_has_function(compiler->program, node->var_name,
                             node->var_name_len)) {
...
static int call_function(VM *vm, Instruction *ins, HDValue *stack, int *sp) {
  ...
  for (int i = 0; i < fn->param_count; i++) {
    EnvDefine(&local, fn->param_names[i], (size_t)fn->param_name_lens[i],
              stack[first + i]);
  }
  *sp = first;
  ...
  while (ip < chunk->count) {
    ...
    case BC_GE:
      ...
      if (!is_numeric(left) || !is_numeric(right)) {
        printf(
            "Runtime error: numeric operator used on a non-numeric value.\n");
        return 0;
      }
    ...
    case BC_CONCAT:
      if (!pop_value(stack, &sp, &right) || !pop_value(stack, &sp, &left))
        return 0;
      if (!push_value(stack, &sp, concat_values(left, right)))
        return 0;
      break;
    ...
    case BC_MAKE_ARRAY: {
      ...
      array.elements =
          (HDValue *)malloc(sizeof(HDValue) * (size_t)ins->operand);
      if (!array.elements && ins->operand > 0)
        return 0;
      int first = sp - ins->operand;
      for (int i = 0; i < ins->operand; i++) {
    ...
    case BC_ARRAY_LEN:
      if (!pop_value(stack, &sp, &value))
        return 0;
      if (value.type == VAL_ARRAY) {
        if (!push_value(stack, &sp, int_value(value.array_len)))
          return 0;
      } else if (value.type == VAL_STRING) {
        if (!push_value(stack, &sp, int_value(value.str_len)))
          return 0;
      } else {
        printf(
            "Runtime error: length requested from non-array/string value.\n");
        return 0;
      }
      break;
    ...
    case BC_ARRAY_GET:
      if (!pop_value(stack, &sp, &right) || !pop_value(stack, &sp, &left))
        return 0;
      if (left.type != VAL_ARRAY || right.type != VAL_INT || right.i64 < 0 ||
          right.i64 >= left.array_len) {
        printf("Runtime error: invalid array access.\n");
        return 0;
      }
      if (!push_value(stack, &sp, left.elements[right.i64]))
        return 0;
      break;
