...
struct netmon {
  ...
  int mouse_x, mouse_y, buttons; /* ui_begin wants these at draw time */
  int list_rows;                 /* PGUP/PGDN step size, set during draw */
...
static void fmt_mac(char *out, size_t cap, const uint8_t *mac) {
  snprintf(out, cap, "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2],
           mac[3], mac[4], mac[5]);
}
...
static void summarize(const struct net_frame *f, char *out, size_t cap) {
  ...
  if ((proto == IP_PROTO_TCP || proto == IP_PROTO_UDP) && l4_len >= 4) {
    snprintf(out, cap, "%s  %s:%u > %s:%u",
             proto == IP_PROTO_TCP ? "TCP" : "UDP", src, be16(l4), dst,
             be16(l4 + 2));
    return;
  }
...
static struct net_frame *frame_at(struct netmon *nm, int index) {
  return &nm->history[(nm->first + index) % HISTORY];
}

static void history_push(struct netmon *nm, const struct net_frame *f) {
  if (nm->count < HISTORY) {
    nm->history[(nm->first + nm->count) % HISTORY] = *f;
    nm->count++;
    return;
  }

  nm->history[nm->first] = *f;
  nm->first = (nm->first + 1) % HISTORY;
...
static void discard_capture(struct netmon *nm) {
  struct net_frame drop[NET_CAPTURE_BATCH];
  for (int round = 0; round < 8; round++)
    if (net_capture(&nm->cursor, drop, NET_CAPTURE_BATCH) < NET_CAPTURE_BATCH)
      break;
}

static void drain_capture(struct netmon *nm) {
  ...
  for (int round = 0; round < 8; round++) {
    uint64_t asked = nm->cursor;
    long n = net_capture(&nm->cursor, batch, NET_CAPTURE_BATCH);
    if (n <= 0)
      break;
    if (batch[0].seq > asked)
      nm->missed += batch[0].seq - asked;
    for (long i = 0; i < n; i++)
      history_push(nm, &batch[i]);
...
static void draw_header(struct gfx_surface *s, struct ui_context *ui,
  ...
  snprintf(line, sizeof(line), "held %d%s", nm->count,
           nm->paused ? "   PAUSED" : "");
  gfx_text(s, in.x + 576, in.y + ROW_H * 2, line, nm->paused ? COL_TX : COL_DIM,
           1);

  if (nm->missed) {
    snprintf(line, sizeof(line), "%llu frames lost to ring overwrite",
             (unsigned long long)nm->missed);
    gfx_text(s, in.x, in.y + ROW_H * 3, line, COL_BAD, 1);
  }
}

static void draw_list(struct gfx_surface *s, struct ui_context *ui,
                      struct gfx_rect r, struct netmon *nm) {
  ui_well(ui, r);
  ...
  for (int i = 0; i < rows - 1; i++) {
    int index = nm->scroll + i;
    if (index >= nm->count)
      break;

    const struct net_frame *f = frame_at(nm, index);
    int y = in.y + (i + 1) * ROW_H;

    if (index == nm->selected)
    ...
    snprintf(line, sizeof(line), "%-8llu %s   %-6u %s",
             ...
             f->direction == NET_DIR_TX ? "TX" : "RX", f->length, summary);
    gfx_text(s, in.x + 4, y, line, f->direction == NET_DIR_TX ? COL_TX : COL_RX,
             1);
  }
...
static void draw_hex(struct gfx_surface *s, struct ui_context *ui,
  ...
  if (nm->selected < 0 || nm->selected >= nm->count) {
    gfx_text(s, in.x + 4, in.y, "no frame selected", COL_DIM, 1);
    return;
  }

  const struct net_frame *f = frame_at(nm, nm->selected);
  struct gfx_rect prev = gfx_clip_push(s, in);
  ...
  if (f->captured < f->length) {
    char note[80];
    snprintf(note, sizeof(note), "%u of %u bytes captured", f->captured,
             f->length);
...
static void clamp_view(struct netmon *nm, int rows) {
  if (nm->count == 0) {
    nm->selected = -1;
    nm->scroll = 0;
    return;
  ...
  if (nm->selected < 0)
    nm->selected = nm->count - 1;
  if (nm->selected >= nm->count)
    nm->selected = nm->count - 1;

  if (nm->selected < nm->scroll)
    nm->scroll = nm->selected;
  if (nm->selected >= nm->scroll + rows)
    nm->scroll = nm->selected - rows + 1;
  if (nm->scroll < 0)
    nm->scroll = 0;
}

static void select_delta(struct netmon *nm, int delta) {
  if (nm->count == 0)
    return;
  ...
  nm->follow = (nm->selected == nm->count - 1);
}

static int netmon_event(struct app *a, const struct wm_event *ev, void *ctx) {
  struct netmon *nm = ctx;
  switch (ev->type) {
  case WM_EV_KEY_DOWN:
    switch (ev->param) {
    case KEY_ESC:
      return APP_ACTION_EXIT;
    case KEY_UP:
      select_delta(nm, -1);
      break;
    case KEY_DOWN:
...
static int netmon_idle(struct app *a, void *ctx) {
  (void)a;
  (void)ctx;
  return APP_ACTION_REDRAW; /* a monitor is always live */
}

static void netmon_draw(struct app *a, void *ctx) {
  struct netmon *nm = ctx;
  struct gfx_surface *s = &a->surface;

  if (nm->paused)
    discard_capture(nm); /* bounded drain-and-drop */
  else {
    drain_capture(nm);
    if (nm->follow && nm->count > 0)
      nm->selected = nm->count - 1;
  }
  ...
  struct gfx_rect header =
      gfx_rect_make(MARGIN, MARGIN, s->w - MARGIN * 2, header_h);
  int list_y = header.y + header.h + MARGIN;
  int hex_y = s->h - MARGIN - hex_h;
  struct gfx_rect list = gfx_rect_make(MARGIN, list_y, s->w - MARGIN * 2,
                                       hex_y - MARGIN - list_y);
  struct gfx_rect hex = gfx_rect_make(MARGIN, hex_y, s->w - MARGIN * 2, hex_h);

  nm->list_rows = (gfx_rect_inset(list, 4).h / ROW_H) - 1;
  if (nm->list_rows < 1)
    nm->list_rows = 1;
  clamp_view(nm, nm->list_rows);

  ui_begin(&nm->ui, s, 0, nm->mouse_x, nm->mouse_y, nm->buttons);
  gfx_clear(s, ui_theme_default.face);
  draw_header(s, &nm->ui, header, &st, nm);
  draw_list(s, &nm->ui, list, nm);
  draw_hex(s, &nm->ui, hex, nm);
  ui_end(&nm->ui);
}

int main(void) {
  /* static: the 512-frame history is far too large for the 8 KiB
   * user stack crt1 hands out. */
  static struct netmon nm;
  nm.selected = -1;
  nm.follow = 1;

  if (app_open_ex(&nm.app, WIN_W, WIN_H, "netmon", WM_CREATE_STATUSBAR) != 0) {
    printf("netmon: app_open failed\n");
    return 1;
  }
  wm_window_set_status(nm.app.window.handle,
                       "ESC quit  UP/DN PGUP/PGDN  P pause  C clear");

  static const struct app_callbacks cb = {
      .event = netmon_event,
      .draw = netmon_draw,
      .idle = netmon_idle,
  };
  return app_run(&nm.app, &cb, &nm);
}

// int main(void) {
//     /* static: struct app carries the 512-frame history, which is far too
//      * large for the 8 KiB user stack crt1 hands out. */
//     static struct netmon netmon_app;
//     netmon_app.selected = -1;
//     netmon_app.follow = 1;

//     app_open(&netmon_app.app, WIN_W, WIN_H, "netmon");
